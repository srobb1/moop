...
/**
 * Parse BLAST XML output to extract text summary
 * Converts XML BLAST results to plain text format for downloading
 * 
 * @param string $xml_output XML output from BLAST
 * @return string Plain text representation of results
 */
function parseBlastXmlToText($xml_output) {
    $text = "BLAST Search Results\n";
    $text .= "=" . str_repeat("=", 78) . "\n\n";
    ...
    try {
        ...
        // Extract query info
        if (isset($xml->BlastOutput_query_def)) {
            $text .= "Query: " . (string)$xml->BlastOutput_query_def . "\n";
        }
        if (isset($xml->BlastOutput_query_len)) {
            $text .= "Query Length: " . (string)$xml->BlastOutput_query_len . " bp\n\n";
        }
        
        // Extract hits
        if (isset($xml->BlastOutput_iterations->Iteration->Iteration_hits->Hit)) {
            $text .= "Hits Found:\n";
            $text .= "-" . str_repeat("-", 77) . "\n\n";
            ...
            foreach ($xml->BlastOutput_iterations->Iteration->Iteration_hits->Hit as $hit) {
                $text .= "Hit #" . $hit_num . ":\n";
                if (isset($hit->Hit_def)) {
                    $text .= "  Definition: " . (string)$hit->Hit_def . "\n";
                }
                if (isset($hit->Hit_accession)) {
                    $text .= "  Accession: " . (string)$hit->Hit_accession . "\n";
                }
                if (isset($hit->Hit_len)) {
                    $text .= "  Length: " . (string)$hit->Hit_len . " bp\n";
                }
                
                // Extract HSPs (alignments)
                if (isset($hit->Hit_hsps->Hsp)) {
                    $hsp_num = 1;
                    foreach ($hit->Hit_hsps->Hsp as $hsp) {
                        $text .= "  Alignment #" . $hsp_num . ":\n";
                        if (isset($hsp->Hsp_evalue)) {
                            $text .= "    E-value: " . (string)$hsp->Hsp_evalue . "\n";
                        }
                        if (isset($hsp->Hsp_bit_score)) {
                            $text .= "    Bit Score: " . (string)$hsp->Hsp_bit_score . "\n";
                        }
                        if (isset($hsp->Hsp_identity)) {
                            $text .= "    Identity: " . (string)$hsp->Hsp_identity . " residues\n";
                        }
                        $hsp_num++;
                    }
                }
        ...
        $text .= "Error parsing results: " . $e->getMessage() . "\n";
    }
