...
if (file_exists($cache_file)) {
    ...
    if ($raw) {
        ...
        foreach ($raw['data'] ?? [] as $_org_data) {
            $_checks = $_org_data['overall_status']['checks'] ?? [];
            if (isset($_checks['assemblies_in_groups']) && !$_checks['assemblies_in_groups']) {
...
// Count stale group entries (in JSON but not on disk at gene_set level)
$_groups_file = $config->getPath('metadata_path') . '/organism_assembly_groups.json';
if (file_exists($_groups_file)) {
    $_gd = json_decode(file_get_contents($_groups_file), true) ?? [];
    foreach ($_gd as $_ge) {
        $_gs = $_ge['gene_set'] ?? 'v1';
...
unset($_gd, $_ge, $_gs, $_gs_path, $_groups_file, $_org_data, $_checks);
