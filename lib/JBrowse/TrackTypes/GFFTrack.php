<?php
/**
 * GFF Track Type for JBrowse2
 * 
 * Handles GFF3 gene annotation tracks with tabix indexing.
 * For explicit GFF tracks (AUTO tracks handled by AutoTrack.php).
 * 
...
class GFFTrack extends BaseTrack implements TrackTypeInterface
{
...
/**
    ...
    /**
     * Validate track data specific to GFF track type
     */
    ...
    public function generate($trackData, $organism, $assembly, $options = [])
    {
        try {
            ...
            error_log("GFF track generation failed: " . $e->getMessage());
            return false;
        }
    }
    ...
    {
        return ['.gff3.gz', '.gff.gz', '.gff3', '.gff'];
    }
     ...
     * Find TBI index file
     */
    private function findTbiIndex(string $gffPath): ?string
    {
        $tbiPath = $gffPath . '.tbi';
        ...
        $tbiPath = preg_replace('/\.gff3?(\.gz)?$/i', '.tbi', $gffPath);
        if (file_exists($tbiPath)) {
            return $tbiPath;
        }
    ...
    private function generateTrackId(string $filePath, string $organism, string $assembly): string
    {
        ...
        $basename = preg_replace('/\.gff3?(\.gz)?$/i', '', $basename);
